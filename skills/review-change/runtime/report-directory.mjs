...
export async function createReportDirectory({
  ...
  for (const candidate of [preferredRoot, fallbackRoot]) {
    ...
    if (safeRoot) return mkdtemp(path.join(safeRoot, "review-change-report-"));
  }
  throw new Error("No safe temporary directory is available for the Review change report");
}
...
function defaultFallbackRoot() {
  return process.platform === "win32" ? path.join(homedir(), ".review-change-tmp") : "/tmp";
}
