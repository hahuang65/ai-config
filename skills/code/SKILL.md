...
2. **For each slice (dependency order)** — work one slice at a time; do NOT batch slices:
   ...
   - **Mark the slice complete** in `tasks.html` by changing each criterion's visible and `data-status` state and the slice's visible and `data-status` state to `complete`, then move on.
     ...
     Never modify the file through a Bash command.
     This forbids every command-line text transformer, not one language: interpreter one-liners and heredocs (`python3`, `perl`, `ruby`, `node`) and stream editors (`sed`, `awk`, `ed`) alike.
     Such commands embed arbitrary transform code, so no permission rule can match them and the run stops at a permission prompt.
