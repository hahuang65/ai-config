...
Comply with the project rules in `rules/` (coding-style, testing, security, performance, git-commit). In Claude Code these are global instructions; in pi, read from `~/.pi/agent/rules/`; in oh-my-pi, load via `rule://<name>` when entering the rule's domain. Testing Decisions must follow the testing rules; Implementation Decisions must respect security and performance rules.
