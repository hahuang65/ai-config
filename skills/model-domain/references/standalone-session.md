...
The session builds or sharpens the project's ubiquitous language and the selected context documentation that records or locates it.
Resolve the main-directory or linked-worktree destination from `SKILL.md` before selecting a mode.

## Select the Mode

Resolve a mode from `$ARGUMENTS` and the selected context documentation:

- **Build from scratch** — bootstrap a project that has no context documentation.
  Use this mode by default when the selected local files do not exist or the selected Confluence context document has no domain model yet.
...
If context documentation exists and the mode is unclear, briefly offer **augment** or **audit and condense**, then wait for the choice.
Do not ask the user for facts that code, documentation, or existing decision records can answer.
Explore those sources first and ask only for domain decisions.

## Build From Scratch

1. Inspect project documentation, code structure, public behavior, and decision records for candidate concepts and context boundaries.
2. If the destination is local and one context clearly fits, use a root `CONTEXT.md`.
   If multiple contexts are already evident, propose a `CONTEXT-MAP.md` and the subordinate context files before writing them.
   If the destination is Confluence, use the supplied context document's single-context or multi-context native structure.
...
1. Read the selected context document or `CONTEXT-MAP.md` when present, all relevant context documentation, applicable decision records, and the matching code and documentation.
...
1. Read the selected context document or `CONTEXT-MAP.md` when present and all context documentation in scope, then compare it with relevant code, product documentation, and decision records.
...
Report the mode, destination, context documentation created or changed, terms added or changed, terms removed or moved, ambiguities resolved, and decision records created.
