...
Write one self-contained HTML Change review report to the operating system temp directory.
...
Render each Finding card's primary `path:line` prominently.
...
Do not duplicate or guess its CLI commands in Change review.
...
When `CHANGE_REVIEW_GATE=1`, write the report into the provided report root, report its path, and return after the report stage; the parent CLI validates and launches the single generated HTML file in a new Firefox window through a `file:` URL on macOS or uses the platform HTML viewer elsewhere.
