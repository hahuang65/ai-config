...
class TerminalStatus {
  ...
  start(context = {}) {
    ...
    if (!this.fullScreen) return this.emitLine("Change review");
  ...
  emitFinalLine(exitCode) {
    ...
    this.emitLine(`${symbol} Change review ${outcome} ${elapsed(this.state.startedAt, this.now())}`);
  }
}
...
function applyPiEvent(state, event, timestamp, line) {
  ...
  if (event.type === "tool_execution_start" && event.toolName === "change_review_status") {
...
function summaryMarkdown(parentSummary, assistantSummary) {
  ...
  const outcome = lines.shift() ?? "Change review";
