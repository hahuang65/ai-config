export function buildChangeReviewPrompt({ target, intent, scopeKind, sourceScopeResolved, skillDirectory }) {
  const invocation = JSON.stringify({ target, intent, scopeKind, sourceScopeResolved });
  return [
    "Act as the outer standalone Change review driver.",
    `Load and execute the change-review skill from ${JSON.stringify(skillDirectory)}.`,
    ...
    "Use change_review_status to mark start and completion or failure for review, evidence, documentation, lint, and report in that exact order; call it alone and wait for its successful result before issuing any work for that stage.",
    ...
    "Do not invoke the change-review executable; this process already owns the active gate.",
