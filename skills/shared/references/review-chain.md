...
5. **Fact-check** — run the `fact-checker` skill on both `prd.md` and `tasks.md`. This verifies that claims (module names, decisions, behavior descriptions) match what was actually implemented. Update either document if it drifted.

6. **Refresh visuals** — regenerate `prd.html` and `tasks.html` so they mirror the final markdown, and open them in the browser. Mandatory — the visuals MUST always mirror the markdown.

7. **Diff review** — if `visual-explainer` is available, generate `diff-review.html` via `/diff-review`: compare the working tree against the branch point (typically `main`). Open it in the browser, then run the `fact-checker` skill on the generated HTML. If `visual-explainer` is not available, skip silently.

8. **Verify task-to-implementation sync** — every acceptance criterion checked off (`- [x]`), every slice marked `**Status:** ✅ Complete`, deviations documented in the slice body, and both `tasks.html` and `prd.html` reflecting the final state.
