...
test_phase_review_change() {
  ...
  check_content_cached "$content" "$label" "initial adversarial review.*final convergence review.*fresh.*complete.*change-reviewer"
  check_content_cached "$content" "$label" "[Ii]ntermediate source or test repairs.*targeted rereview.*current-round delta.*dependency closure"
  check_content_cached "$content" "$label" "coherent repair batches.*shared component.*interface.*invariant"
  check_content_cached "$content" "$label" "[Rr]un distinct batches sequentially.*latest working state"
  check_content_cached "$content" "$label" "[Dd]o not run a full review between coherent repair batches"
  check_content_cached "$content" "$label" "Every manifest.*complete changed-file list.*content identit"
  check_content_cached "$content" "$label" "targeted Review manifest.*current-round delta.*dependency closure"
  check_content_cached "$content" "$label" "Inspection ledger.*unchanged content.*same identity.*another read"
  check_content_cached "$content" "$label" "targeted rereview.*clean.*ask-user.*no progress.*round limit.*final convergence review"
  check_content_cached "$content" "$label" "Source or test changes restart.*targeted adversarial rereview.*final complete review.*no further repair.*regardless.*targeted outcome"
  check_content_cached "$content" "$label" "prior Inspection ledger.*coverage routing data.*not.*reviewer reasoning.*fixer rationale"
  check_content_cached "$content" "$label" "capture.*round-start.*content identities.*derive.*actual before/after"
  check_content_cached "$content" "$label" "recoverable.*round-start.*content.*workflow-owned temporary.*derive.*delta"
  check_content_cached "$content" "$label" "path state.*present.*deleted.*renamed.*tombstone"
  check_content_cached "$content" "$label" "complete review.*base.*current.*targeted rereview.*round-start.*post-round"
  check_content_cached "$content" "$label" "complete Review manifest.*specialist Findings.*targeted.*exclude.*evidence summaries"
  check_content_cached "$content" "$label" "prior Finding records.*stable ID.*original anchor.*source-verifiable invariant"
  check_content_cached "$content" "$label" "targeted Review manifest.*repaired Finding IDs.*prior Finding records.*decision ledger projection"
  check_content_cached "$content" "$label" "targeted.*decision ledger projection.*explicit user dispositions.*never.*material-change judgment"
  check_content_cached "$content" "$label" "prior Finding record.*complete or targeted.*final complete.*independent"
  check_content_cached "$content" "$label" "final convergence.*manifest.*all complete-review routing fields"
  check_content_cached "$content" "$label" "final complete.*exclude.*decision ledger.*prior Finding records.*unresolved Findings"
  check_content_cached "$content" "$label" "orchestrator.*reconcile.*dispositions.*after.*independent.*result"
  check_content_cached "$content" "$label" "reconcile.*stable Finding ID.*invariant.*anchor lineage.*current source state"
  check_content_cached "$content" "$label" "final complete.*repair-eligible Finding.*repair budget.*repair round"
  check_content_cached "$content" "$label" "user-authored Finding.*exact.*anchor.*acceptance statement.*prior Finding record"
  check_content_cached "$content" "$label" "round limit.*explicit human authorization.*new bounded cycle"
  check_content_cached "$content" "$label" "Inspection ledger.*neutral.*interface.*invariant identifiers.*never.*conclusion.*evidence.*judgment"
  check_content_cached "$content" "$label" "source or test.*targeted rereview.*documentation-only.*documentation check.*formatting-only.*lint"
  check_content_cached "$content" "$label" "[Rr]equire each Change reviewer.*inspect changed hunks"
  check_content_cached "$content" "$label" "orchestrating session.*consume.*without substituting"
...
test_agent_change_reviewer() {
  ...
  check_content_cached "$content" "$label" "[Cc]omplete review.*targeted rereview"
  check_content_cached "$content" "$label" "Classify severities.*error.*warning.*info.*actions.*auto-fix.*ask-user.*no-op"
  ...
  check_content_cached "$content" "$label" "targeted rereview.*prior Inspection ledger.*coverage routing data"
  check_content_cached "$content" "$label" "targeted rereview.*prior Finding records.*stable ID.*original anchor.*source-verifiable invariant"
  check_content_cached "$content" "$label" "[Vv]erify.*prior Finding.*exact invariant.*boundary.*absence.*type"
  check_content_cached "$content" "$label" "[Ff]or each repaired Finding.*repair-caused regression.*tests.*dependency closure.*missing regression coverage"
  check_content_cached "$content" "$label" "Inspection ledger.*neutral routing labels.*never.*behavior conclusions.*evidence summaries.*Finding judgments.*rationale"
  check_content_cached "$content" "$label" "specialist Findings.*complete review.*targeted rereview.*exclude.*evidence.*repair direction"
...
test_agent_change_fixer() {
  ...
  check_content_cached "$content" "$label" "coherent repair batch.*shared component.*interface.*invariant"
  check_content_cached "$content" "$label" "[Rr]epair batches run sequentially.*current working state"
  check_content_cached "$content" "$label" "affected_interfaces.*changed entry points.*callers.*tests.*invariants"
  check_content_cached "$content" "$label" "changed_files.*intentionally changed"
  check_content_cached "$content" "$label" "verification.*exact focused check"
}
