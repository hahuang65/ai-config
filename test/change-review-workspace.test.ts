...
import {
  ...
  safeRemoteUrl,
} from "../skills/change-review/runtime/workspace.mjs";
...
describe("standalone Change review remote metadata", () => {
...
describe("standalone Change review workspace", () => {
  ...
  test("snapshots tracked and untracked working state into an isolated clone", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "change-review-source-"));
    const treehouse = await mkdtemp(path.join(tmpdir(), "change-review-treehouse-"));
    cleanups.push(async () => {
