...
describe("review-change CLI prompt", () => {
  ...
  test("treats target and intent as data while preserving read-only boundaries", () => {
    ...
    expect(prompt).toContain("This report is presentation-only");
    ...
    expect(prompt).toContain("Write every surfaced Finding in plain language");
    expect(prompt).toContain("lead with the concrete impact and recommended change");
    expect(prompt).toContain("keep exact machine values and paths secondary");
    ...
    expect(prompt).toContain("include no Review publication, selection form, signed publication envelope, or provider action");
    expect(prompt).not.toContain("copyable general-review Markdown");
    expect(prompt).not.toContain("copyable Markdown block per Finding");
