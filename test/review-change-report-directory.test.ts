...
import { createReportDirectory } from "../skills/review-change/runtime/report-directory.mjs";
...
describe("Review change report directory", () => {
  test("rejects a preferred temp root inside the source checkout", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "review-change-report-test-"));
    roots.push(root);
