...
import { parseArguments } from "../skills/change-review/runtime/arguments.mjs";
import { buildChangeReviewPrompt } from "../skills/change-review/runtime/prompt.mjs";
import { createLifecycleCancellation } from "../skills/change-review/runtime/lifecycle-cancellation.mjs";
import { renderMarkdownWithGlow } from "../skills/change-review/runtime/markdown-summary.mjs";
import { openReportArtifact, runChangeReview, spawnInForeground } from "../skills/change-review/runtime/runner.mjs";
import { createTerminalStatus } from "../skills/change-review/runtime/status.mjs";
import { renderBoundaryFailureSummary } from "../skills/change-review/runtime/summary.mjs";
import { assertSupportedNode } from "../skills/change-review/runtime/version.mjs";
...
describe("change-review CLI runtime", () => {
  ...
  test("renders a sanitized boundary summary before the review lifecycle exists", () => {
    ...
    expect(summary).toContain("Change review failed with exit 1");
    expect(summary).not.toContain("secret-token");
  });

  test("prints a boundary summary for nested-gate rejection", async () => {
    const executable = path.resolve(import.meta.dir, "../skills/change-review/bin/change-review.mjs");
    const child = Bun.spawn([process.execPath, executable], {
      env: { ...process.env, CHANGE_REVIEW_GATE: "1" },
      stdout: "pipe",
      stderr: "pipe",
    });
    ...
    expect(stdout).toContain("Change review failed with exit 1");
    expect(stdout).toContain("a Change review gate is already active");
  });
  ...
  test("renders readable lifecycle and sub-stage logs without terminal control sequences", () => {
    ...
    status.setReportPath("/tmp/change-review.html");
    ...
    expect(output).toContain("✓ Change review completed (1.8s)");
    expect(summary).toContain("Change review completed");
    expect(summary).toContain("Report: /tmp/change-review.html");
  ...
  test("keeps left-pane sub-stages concise and shows the review worktree", () => {
    ...
    status.start();
    status.setWorkspacePath("/Users/example/.review-treehouse/project-change-review-cli-1234");
    ...
    expect(frame).toContain("WORKTREE /Users/example/.review-treehouse/project-change-review-cli-1234");
  ...
  test("records bounded bash exit evidence in the owning stage log", () => {
    ...
    status.piEvent({ type: "tool_execution_start", toolCallId: "bash-1", toolName: "bash", args: {
      command: "bun test test/change-review-cli.test.ts",
    } });
  ...
  test("supports full-screen navigation and final Summary-stage dismissal", async () => {
    ...
    status.setReportPath("/tmp/change-review-summary.html");
    status.piEvent({ type: "message_end", message: {
      role: "assistant",
    ...
    expect(output).toContain("Change review failed with exit 130");
    expect(output).toContain("Report: /tmp/change-review-summary.html");
  ...
  test("renders Markdown with Glow inside the scrollable final Summary stage", async () => {
    ...
    expect(markdown).toContain("# Change review completed");
    ...
    expect(resizingFrame).toContain("Change review completed");
  ...
  test("renders a full-screen pipeline and restores the terminal", () => {
    ...
    status.piEvent({ type: "tool_execution_start", toolName: "bash", args: {
      command: "bun test test/change-review-target.test.ts",
    } });
    status.finish(0);
    ...
    expect(output).toContain("bun test test/change-re");
    expect(output).toContain("\u001b[1m\u001b[36mCHANGE REVIEW");
    ...
    expect(output).toContain("Change review completed");
    expect(output).toContain("\u001b[?1049l");
  });
});

describe("change-review CLI arguments", () => {
...
describe("change-review CLI prompt", () => {
  test("treats target and intent as data while preserving read-only boundaries", () => {
    const prompt = buildChangeReviewPrompt({
      ...
      skillDirectory: "/skills/change-review",
    ...
    expect(prompt).toContain("change_review_status");
...
describe("change-review CLI runner", () => {
  test("opens the single HTML report through a Firefox Apple event and verifies completion", async () => {
    const reportRoot = await mkdtemp(path.join(tmpdir(), "change-review-open-"));
    const reportPath = path.join(reportRoot, "change-review.html");
    await writeFile(reportPath, "<!doctype html><title>Change review</title>");
  ...
  test("launches pi in an isolated foreground workspace", async () => {
    ...
    let openedRoot = "";
    let cleaned = false;
    const exitCode = await runChangeReview(
      {
        target: "main...HEAD",
      ...
      },
      {
        ...
        openReport: async (reportRoot) => {
          ...
          return `${reportRoot}/change-review.html`;
        },
        spawnProcess: (command, args, options) => {
          invocation = { command, args, options };
    ...
    expect(invocation?.args).toEqual([
      ...
      "--no-session",
      "--skill",
      path.resolve("/skills/change-review"),
      expect.stringContaining('"target":"main...HEAD"'),
    ]);
    expect(invocation?.options).toMatchObject({
      ...
      env: {
        ...
        CHANGE_REVIEW_GATE_ROOT: "/isolated",
        CHANGE_REVIEW_REPORT_ROOT: "/reports/session",
        CHANGE_REVIEW_SUBAGENT_MODEL: "openai/review-model",
      },
    });
  });
  ...
  test("reports lifecycle status while keeping setup outcomes concise", async () => {
    ...
    await runChangeReview(
      { target: "feature/cli", intent: null, piOptions: [] },
      {
        ...
        createReportDirectory: async () => "/reports/session",
        openReport: async () => "/reports/session/change-review.html",
  ...
  test("cancels target resolution and restores the full-screen terminal on interruption", async () => {
    ...
    const completion = runChangeReview(
      { target: "feature/cli", intent: null, piOptions: [] },
      {
        environment: {},
        processRef,
    ...
    expect(rawModes).toEqual([true, false]);
    expect(output).toContain("Change review failed with exit 143");
    expect(output).toContain("\u001b[?1049l");
    expect(summary).toBe("");
    expect(processRef.listenerCount("SIGTERM")).toBe(0);
  });
  ...
  test("rejects nested gate execution before creating a workspace", async () => {
    await expect(runChangeReview(
      { target: null, intent: null, piOptions: [] },
      { environment: { CHANGE_REVIEW_GATE: "1" }, skillDirectory: "/skills/change-review" },
