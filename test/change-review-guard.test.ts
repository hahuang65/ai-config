...
import { evaluateChangeReviewToolCall } from "../harnesses/pi/extensions/change-review-guard.ts";

const outerContext = { active: true, root: "/repo", tempRoot: "/tmp" };

describe("Change review CLI guard", () => {
  test("does nothing outside an active CLI gate", () => {
    expect(evaluateChangeReviewToolCall(
  ...
  test("blocks repository and external writes but allows temporary reports", () => {
    ...
    expect(evaluateChangeReviewToolCall(
      { toolName: "write", input: { path: "/tmp/change-review-report.html" } },
      outerContext,
    )).toBeNull();
  });
