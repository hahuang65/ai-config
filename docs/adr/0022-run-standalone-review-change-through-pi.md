...
While the final Summary is open, Cleanup remains pending and the isolated workspace plus full telemetry log still exist.
After dismissal, the parent restores the terminal, closes telemetry, removes only the exact isolated workspace, and reports the final Cleanup outcome outside the full-screen Summary as `Removed` on success.
It owns cancellation from terminal setup through final Summary dismissal, propagates one AbortSignal into setup subprocesses, latches interruption received while initial Glow rendering is pending, dismisses the final view through normal teardown on an external signal, restores terminal state on interruption, and always emits a parent summary even when no assistant summary exists.
