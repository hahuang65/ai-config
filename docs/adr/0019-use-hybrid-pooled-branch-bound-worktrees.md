...
The worktree manager gives each repository a project group beneath `~/.treehouse/`, with readable active task directories by intent and detached available slots under that group's hidden `.pool/` directory.
...
Ordinary work defaults to a local task branch and uses a worktree only when explicitly requested; `/build` uses a linked worktree whenever a compatible Treehouse executable is available, and a local task branch can be converted later without changing its branch identity.
...
Treehouse owns the worktree transition after creating, converting, or selecting a task worktree: its harness adapters continue the current visible interaction through native worktree or session-switching capabilities, while its terminal surface enters through a shell.
Claude Code records a live process ownership claim, enters through its native existing-worktree operation, returns through its native exit operation with keep behavior, and releases the exact claim after restoring the original main directory without allowing Claude Code to remove a Treehouse-owned worktree.
...
The `/build` orchestrator continues uninterrupted inside any existing linked worktree without requiring Treehouse, but delegates acquisition and transition to Treehouse when started outside one and executable preflight succeeds.
If the executable is missing or protocol-incompatible before acquisition, `/build` warns that isolation and Treehouse merge support will be unavailable and asks whether to continue through the ordinary local-task-branch workflow.
...
Acquisition or native-transition failure after Treehouse mutates state preserves the task worktree and stops without offering branch fallback.
Standalone Treehouse and `/merge` workflows fail closed when the executable is unavailable or incompatible.
Treehouse never adopts or relocates an unmanaged worktree and contains no build-phase knowledge.
...
The Treehouse executable, lifecycle service, runtime modules, CLI tests, and command installation are baseline Git tooling owned by the Git dotfiles repository.
The AI dotfiles repository owns only Treehouse workflow skills, build and commit policy, and native Claude Code and pi transition adapters.
...
The Git dotfiles installer exposes the executable at `~/.local/bin/treehouse`; the AI installer does not install it.
...
Without those global aliases it uses ordinary unprefixed branch creation and explicit fast-forward synchronization; optional per-project pool capacity comes from `TREEHOUSE_MAX_TREES` with a default of four.
A successful merge runs from the main project directory and is fast-forward-only; if trunk cannot advance directly to the feature tip, the manager alerts the user without changing either checkout or the caller's location.
...
The harness-neutral `/merge` skill is a thin alias that forwards its arguments to the Treehouse merge flow; it never implements Git integration independently, falls back to raw `git merge`, or pushes.
It refuses outside a managed task worktree rather than guessing at a target.
The `/commit` skill remains checkout-local: it may inspect state, stage the selected change, and create one commit, but never invokes Treehouse, changes worktree or branch lifecycle, merges, rebases, pushes, or chains into `/merge`.
...
This combines Treehouse-style environment reuse with ordinary Git branch recoverability and avoids the orphaned-commit risk of a purely detached pool.
