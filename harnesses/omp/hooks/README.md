...
TS/JS modules that subscribe to oh-my-pi's runtime events via the `HookAPI` from
`@oh-my-pi/pi-coding-agent/extensibility/hooks`. Per oh-my-pi's hooks doc, the
...
`~/.omp/agent/hooks/`. README files and other non-code are skipped so oh-my-pi's
...
`HookAPI` from the oh-my-pi hooks package, and follow the `guard-*` /
