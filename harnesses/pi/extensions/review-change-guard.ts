...
export interface ReviewChangeGateContext {
  active: boolean;
  root: string;
  tempRoot: string;
}
...
export function evaluateReviewChangeToolCall(
  call: ToolCall,
  ...
  if (resolvedCandidate && isWithinRoot(resolvedCandidate, context.root)) {
    return block("Standalone Review change blocks repository writes in its read-only workspace");
  }
...
function classifyShellMutation(command: string): string | null {
  ...
  if (hasOutputRedirection(command)) {
    return "Standalone Review change blocks shell output redirection outside its report tool path";
  }
  for (const rawSegment of shellSegments(command)) {
    const segment = unwrapCommand(rawSegment);
    if (!segment) return "Standalone Review change blocks an unsupported shell wrapper";
    const executable = path.basename(segment[0] ?? "").toLowerCase();
    if (executable === "git" && isGitMutation(segment.slice(1))) {
      return "Review change blocks Git delivery mutation";
    }
    if (isProviderMutation(executable, segment.slice(1))) {
      return "Review change blocks provider mutation";
    }
...
export default function (pi: ExtensionAPI): void {
  const context: ReviewChangeGateContext = {
  ...
  pi.on("tool_call", (event) => {
    const verdict = evaluateReviewChangeToolCall({
