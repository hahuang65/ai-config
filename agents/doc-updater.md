...
Before starting work, read the following rule files from `rules/` in the repo root (or your harness's global rules directory: `~/.claude/rules/` for Claude Code, `~/.omp/agent/rules/` for oh-my-pi). These are non-negotiable constraints. All documentation you produce MUST comply.
